...
def _contain_info_dir(item: str, target_dir: str) -> str:
    ...
    """
    pattern = re.compile(r"([a-z0-9_.]+)-([a-z0-9_.!+-]+\.)", re.IGNORECASE)
    if pattern.match(item):
        raw_name = item.partition(".")[0].partition("-")[0]
        dist_info_re = re.compile(rf"{re.escape(raw_name)}-([a-z0-9_.!+-]+\.dist-info$)", re.IGNORECASE)
        egg_info_re = re.compile(rf"{re.escape(raw_name)}-([a-z0-9_.!+-]+\.egg-info$)", re.IGNORECASE)
